...
#1 - run scripts
    #1) run once: identify key providers 
        source(file.path(spath, "1 - Identify key providers.R"))
    #2) run once: determine opiates in prescriptions
        source(file.path(spath, "2 - Determine which prescriptions are opiates.R"))
    #3) run once: clean/prep encounter file
        source(file.path(spath, "3 - Prep encounter data.R"))
    #4) run once: clean/prep provider file
        source(file.path(spath, "4 - Prep provider data.R"))
    #5) run once: prep clinic data
        source(file.path(spath, "5 - Prep clinic data.R"))
